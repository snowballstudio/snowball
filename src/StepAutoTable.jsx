      ...
      <div className="stepAutoScroll">
        <div className="stepAutoTable">
          <div className="stepAutoRow stepAutoHeader">
            ...
            <span>安卓 KM</span><span>华为 KM</span><span>苹果 KM</span>
            <span>计算里程</span><span>当日里程</span>
          </div>
          {rows.map((row, index) => (
            <div className="stepAutoRow" key={`${row.date}-${index}`}>
              ...
              <span>{kmText(row.healthConnectKm)}</span>
              <span>{kmText(row.huaweiHealthKitKm)}</span>
              <span>{kmText(row.appleHealthKitKm)}</span>
              <span>{kmText(row.calculatedKm)}</span>
              <span className="stepAutoStrong">{kmText(row.dailyKm)}</span>
            </div>
          ))}
