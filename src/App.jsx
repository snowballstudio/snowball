...
const WORLD_PLACES = [
  ...
  '中国', '澳洲', '新西兰', '日本', '香港','台湾','韩国', '新加坡', '马来西亚', '泰国', '越南', '印度', '菲律宾', '印尼', '尼泊尔', '沙特', '土耳其',
...
const FOOTPRINT_POSITIONS = {
  world: {
    ...
    中国: { x: 77, y: 45 }, 澳洲: { x: 84, y: 76 }, 新西兰: { x: 96, y: 91 }, 日本: { x: 88, y: 43 }, 香港: { x: 78, y: 52 }, 台湾: { x: 83, y: 52 }, 韩国: { x: 84, y: 45 }, 新加坡: { x: 75, y: 67 }, 马来西亚: { x: 74, y: 64 }, 泰国: { x: 72, y: 59 }, 越南: { x: 75, y: 57 }, 印尼: { x: 82, y: 65 }, 菲律宾: { x: 82, y: 58 }, 印度: { x: 71, y: 54 }, 尼泊尔: { x: 73, y: 49 }, 沙特: { x: 59, y: 52 }, 土耳其: { x: 58, y: 44 },
...
const FOOD_ALIAS = {
  ...
  辣椒茄子: ['辣椒', '青椒', '红椒','黄椒','红辣椒','青辣椒','黄辣椒','灯笼辣椒','尖椒','螺丝椒','番茄炒蛋','杭椒','灯笼椒','朝天椒','菜椒','尖辣椒',,'辣椒炒肉','茄子','番茄','西红柿','柿子'],
  ...
  其它蔬菜: ['菜叶', '韭菜','蔬菜','笋','冬笋','莴笋','扁尖','蔬菜色拉','色拉','茭白','苦芥菜','香菜','包菜','金针菜','手撕包菜','甜菜'],
  其它水果: ['榴莲', '菠萝', '凤梨','果汁','苹果汁','石榴','大枣', '枣子', '甜枣', '石榴汁','胡萝卜汁','椰子汁','椰子水','椰子','芒果','芒果汁','橙汁','鲜榨饮料','果粒橙','枇杷','神仙果','无花果','荔枝','牛油果', '车厘子', '热情果', '圣女果','山竹','樱桃','梨', '鸭梨', '杨梅', '香梨', '火龙果', '猕猴桃'],
...
function rangeAverageDivisor(records = [], range = 'today') {
  ...
  return Math.max(1, records.length || ((range === 'week' || range === 'previousWeek') ? 7 : 30))
}
  ...
  */
  ...
  return Object.entries(NUTRITION_TYPES).map(([key, label]) => {
    ...
    return {
      ...
      value,
      level,
      display: range === 'today' || range === 'yesterday' ? `${Math.round(value)}种` : `${value.toFixed(1)}种`,
  ...
  */
  ...
  async function importUserRecords() {
    try {
      ...
      // 导出的饮食/心情只保存用户原始文字，不保存系统派生关键词。
      // 因此导入完成后，仅针对本次备份中涉及的日期，使用当前版本唯一的
      // deriveConversationFields() 重新计算 food / taste / mood / brainPercent，
      // 再写回对应日常记录。步数、离机、屏幕时间等其它字段保持原值。
      const importedDailyDates = Array.isArray(result.importedDailyDates)
        ? [...new Set(result.importedDailyDates.map(formatDateForDaily))]
        : []

      let nextData = result.nextData
      let nextRecords = Array.isArray(nextData.records) ? [...nextData.records] : []
      let recalculatedDaily = 0
      let importedTodayFields = null

      for (const importedDate of importedDailyDates) {
        const conversationRecord = await readConversationRecord(importedDate)
        const derived = deriveConversationFields(conversationRecord)
        const existing = dailyRecordForDate(nextRecords, importedDate) || emptyDailyRecord(importedDate)
        const updated = {
          ...existing,
          food: derived.food || '',
          taste: derived.taste || '',
        ...
        updated.healthy = dailyRecordHealthy(updated)
        nextRecords = mergeDailyRecord(nextRecords, importedDate, updated)
        recalculatedDaily += 1

        if (dateKey(importedDate) === dateKey(todayText())) {
          importedTodayFields = {
      ...
      window.alert(
        `导入完成：饮食/心情新增 ${s.dailyAdded || 0} 天、合并 ${s.dailyMerged || 0} 天，并重新计算 ${recalculatedDaily} 天；足迹新增 ${s.footprintsAdded || 0} 条；物品新增 ${s.thingsAdded || 0} 条；人物新增 ${s.peopleAdded || 0} 条。重复记录已自动跳过。`
      )
    } catch (error) {
